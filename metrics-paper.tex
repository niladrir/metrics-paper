...
Binned distance is highly susceptible to small differences in values and depends on the number of bins as well the \red{starting value of the initial bin (*** check the correct name)}. We initially considered variations on this distance, using kernel density estimates and Hausdorff distance (\cite{huttenlocher:1993}), but the simpler metric was faster to calculate and performed as well as the others as a rough, generic measure of similarity of plots.
