...
\begin{figure*}[hbtp]
\centering
\subfigure[ Dataset X with two variables X1 and X2]{
\includegraphics[scale=0.55]{dat-example-1.pdf}
\includegraphics[scale=0.55]{bin-example-2.pdf}

\label{type_1}
}
\subfigure[ Dataset Y with permuted X1 and original X2]{
...
{\color{red} 1.  In visual inference we compare the test statistic to a finite number of null plots which may affect the decision of a human judge. Distance metrics allows us to get the measure of the quality of a lineup. Distance metrics allows us to provide a range of comparisons to the human subjects based on the difficulty. This may also help us to select the number of human subjects to show a particular lineup.\\

2.  The presence of graphical elements in a lineup plot may affect the quality of the lineup. Distance metrics are developed which takes into account the presence of graphical elements for the common scenarios like a regression line overlaid, a boxplot instead of a dot plot and presence of clusters. An universal distance metric is also described which works for any type of data. The results show that the performances are comparable but the distance metrics which takes into account the graphical elements performs better.\\

3.  A method for obtaining the distribution of the distance metrics is described. For a given lineup, the distribution can be obtained and the statistic for the lineup can be compared to the distribution.\\
 }
