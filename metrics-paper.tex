...
This is two way process. As metrics can be used to measure the quality of a lineup, the lineup protocol also provides an opportunity to measure the performance of a metric. Human eye can pick up unexpected patterns in a plot but metrics are designed to identify particular characteristics in a plot.  Describing plots numerically, is something  of an oxymoron, it cannot be done. Nonetheless, the distance measures provide indications of the quality of a lineup.The scagnostic measures are developed to assess the different aspects of scattered points like outliers, shape, trend, density and coherence. The lineup protocol provides an way to evaluate the performance of each of these measures.
...
Historically in all the lineup experiments done so far, the subjects had to report the reason behind the choice of a particular plot along with the response. Researchers relied on these verbally reported reasons to understand what pattern in the plot triggered the response. Metrics can be used as a more concrete method to obtain this information. 
