...
\green{Bootstrap method is not typically used as a null-generating mechanism. If bootsrap is used to generate null plots from the data plot, most of the null plots will be identical due to overplotting. Also, due to overplotting, the null plots will have fewer data points than the data plot, which may lead the human subject to pick the data plot as different.}
