...
using Amazon's Mechanical Turk~\citep{turk} (MTurk), with some used for evaluating the power of visual inference against that of classical tests \cite{majumder:2011}, \hh{some for comparison of different designs \cite{hofmann2012graphical, loy:2015}, or for targeted conclusions based on visual inference \cite{tengfei:2013}.}
...
\item {\bf Distance based on boxplots (BX):} Let $X_1$ be a categorical variable representing the $J$ groups in the data and $X_2$ be a continuous variable. 
\hh{We calculate group-based minimal statistics of $X_2$ for each level of $X_1$ as \[
d_q(X_1, X_2=j) = (q_1, m, q_3), \text{ where } 1 \le j \le J
\]
where $q_1, m,$ and $q_3$ are the first, second, and third quartile of the subset of $X_2$, where $X_1$ is equal to the $j$th group.
}
