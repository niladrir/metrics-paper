...
%\hh{XXX}
Figure~\ref{turk2comp} shows the \hh{detection rate of each lineup against $\delta$-differences (top) and $\gamma$-number of extreme nulls (b) based on regression and binned (2,2) distance. }
...
A few lineups have a $\delta$-difference close to zero -- for those lineups detection rates of lineups flip from being close to zero to close to one within a very short interval. 
...
<<largep-mtime, echo=FALSE, dependson='t7-distr', out.width='0.7\\textwidth', fig.width=8.5, fig.height=3.5>>=
### Facetted Plots
...
<<lp-exc-t7, echo=FALSE, fig.width=6, fig.height=6, out.width='0.6\\textwidth'>>=
...
names(dt) <- c(".sample", "picks")
picks <- data.frame(.sample=1:20)
picks <- merge(picks, dt, by=".sample")
picks$picks[is.na(picks$picks)] <- 0
