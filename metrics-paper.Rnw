...
\if0\blind
{
  \title{\bf Utilizing Distance Metrics on Lineups to Examine What People Read From Data Plots}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf Utilizing Distance Metrics on Lineups to Examine What People Read From Data Plots}
\end{center}
  \medskip
} \fi
...
\citet{buja:2009} introduced two protocols which bridge the gulf between traditional statistical inference and exploratory data analysis. These are the Rorschach and the lineup protocols. \hh{The Rorschach protocol consists of a set of $m$ (usually, $m=20$) plots rendered from data that is consistent with a given null model.} That way, the Rorschach protocol helps to understand the extent of randomness \hh{in the null model}. The lineup protocol places a statistical plot firmly in the framework of hypothesis tests: a plot of the data is considered to be the test statistic. Unlike the simpler numeric test statistics in classical inference, plots as  test statistics are complex entities. In a lineup, the data plot is placed randomly among a  set of  null plots generated from a null model.
...
Figure~\ref{lineup-example} is an example of a lineup \hh{where a more focussed test is considered}. Suppose we have the following statistical model
...
@
\hh{For the example of this lineup \Sexpr{sum(lpu$response)} out of \Sexpr{nrow(lpu)} observers correctly identify plot~\#2 as the data plot, providing very strong evidence of a linear relationship.}
...
In traditional hypothesis testing, the sampling distribution of a test statistic is functional and continuous. In the lineup protocol, although conceptually we may have an infinite collection of plots from the null distribution, in practice, we can only evaluate against a finite number of null plots. A human judge has a physical limit on the number of plots they can peruse. This poses one of the issues with using the lineup protocol.  Figure~\ref{compare} illustrates the difference. In traditional inference, the black curve represents the sampling distribution for the $t$-distribution under the null hypothesis, and the shaded red area shows the rejection region. In visual inference, let us consider that the black curve gives the sampling distribution although the sampling distribution is essentially a distribution of null plots. 
\hh{While the test statistic is not numeric, we can calculate the value of the statistic from the data underlying the true plot. These values are shown using the red bar (data plot) and the blue bars (null plots) in Figure~\ref{compare}.}
%\hh{XXX why although? I'm afraid I don't understand, where the next sentence is trying to go.} Although the test statistic is not numeric, the true data plot which is the test statistic is represented using red bar and the null plots that are drawn from the null distribution are the blue bars. % for the plots in the lineup shown in Figure~\ref{lineup}. 
...
Various experiments employing the lineup protocol have been conducted 
%There have been eleven experiments conducted
using Amazon's Mechanical Turk~\citep{turk} (MTurk), with some used for evaluating the power of visual inference against that of classical tests \citep{majumder:2011}, \hh{some for comparison of different designs \citep{hofmann2012graphical, loy:2015}, or for targeted conclusions based on visual inference \citep{tengfei:2013}.}
%and the others using the protocol for different purposes. 
It is possible to try the tasks provided to Turkers \citep{majumder:2013}. For evaluating the distance metrics we used the data collected on experiments~1, 2, and~7. Table \ref{tbl:visual_stat} describes the experiments.
...
\hline 
1  & Box plot & \begin{minipage}[c]{2cm} \begin{center}	\scalebox{0.12}{\includegraphics{stat_category1.pdf}} \end{center} \end{minipage} & Which set of box plots shows biggest vertical difference 
between group A and B? \\
2 &  Scatter plot & \begin{minipage}[c]{2cm}  \begin{center} \scalebox{0.3}{\includegraphics{stat_beta_k1.png}} \end{center} \end{minipage} & Of the scatter plots below which one shows data that has steepest slope? \\
  7 & Group separation & \begin{minipage}[c]{2cm} \begin{center}  \scalebox{0.4}{\includegraphics{stat_separation.pdf}} \end{center} \end{minipage} & Which of these plots has the most separation between the coloured groups?  \\
\hline 
\end{tabular}
...
Alternatively, interpoint distance metrics might be adapted to measure distances between plots. For example, when the purpose is differences between groups in a single plot, like side-by-side boxplots, a distance metric that focuses on group separation calculated on each data set might be useful. Bhattacharyya distance \citep{bhattacharyya:1946} is widely used in image processing, for feature extraction. The R package {\tt fpc} \citep{hennig:2010} contains many different ways to calculate distances between groups. 

Ultimately, a very simple binned distance was used in the analyses of the MTurk data, which worked fairly well in most circumstances. However, it was clear immediately that plot design, and the question asked, has a large impact on how a plot is read, and specific distance metrics designed for specific plot types and tasks are needed. Below is a summary of distance metrics used. For all of these distance measures, let $X$ denote the true dataset with one or two variables. Let $Y$ denote the null dataset obtained by using an appropriate null generating mechanism.
...
Let $X_1$ and $X_2$ be two continuous variables. Let $X_1$ be divided into $p$ bins and $X_2$ divided into $q$ bins. % $(i,j)$-th cell represents the $j$-th bin of $X_2$ corresponding to the $i$-th bin of $X_1$. 
 Let $C(X_1,X_2)$ be defined as a $p \times q$ matrix \hh{consisting of the binned frequency of the joint distribution of $X_1$ and $X_2$, i.e.\ the $(i,j)$-th element of the matrix represents the cell count of the number of points falling in this cell.}
...
\begin{eqnarray}\nonumber
d_{\hbox{BN}}^2 (X, Y) &:=& ||C_X(X_1, X_2) - C_Y(X_1,X_2)||^2 \\ &=& \sum_{i=1}^p \sum_{j=1}^q (C_X(X_{1i},X_{2j}) - C_Y(X_{1i},X_{2j}))^2.\label{eq:BNdist}
\end{eqnarray}
...
This distance can be calculated for univariate continuous data, bivariate data with two categorical variables, or data with one continuous and one categorical variable. For a categorical variable, we choose the number of bins  equal to the number of categories.

Binned distance is highly susceptible to small differences in values and depends on the number of bins as well as the anchor position. It is necessary to find the optimal number of bins in each direction. For our purposes `optimal' was defined as the number of bins that produced the most detectable difference to the observed data plot. The distance was measured between the observed data plot, and the closest null, and compared with the biggest distance between any pair of null plots. Details of these choices on various different data sets can be found in the Appendix.

Several variations to this distance are possible, such as a change to using kernel density estimates, a change from $L_2$ to $L_p$ based distance, or using transformations on the counts. 
\hh{XXX should the next sentence go into the discussion/conclusions?}

Hausdorff distance \citep{huttenlocher:1993} was investigated as a generic distance metric as well, but the binned distance was faster to calculate and performed as well as the Hausdorff as a rough, generic measure of similarity of plots.
...
Let $X_1$ be a categorical variable representing $J$ groups in the data and $X_2$ be a continuous variable. 
...
Then the distance metric is given by
 \begin{equation}\nonumber
d^2_{\hbox{BX}}(X, Y) := || d_q(X) - d_q(Y)||^2 \\=
...
This distance is based on the assumption that %after the boxplots have already been constructed, the 
subjects only consider the difference in the boxes to make the distinction. Variations on this might include adding whiskers' values, (number of) outliers, or including higher-order letter values \citep{tukey:1977, lvplot}. 

\item {\bf Distance based on the regression line (RG):}
Let $X_1$ and $X_2$ be two continuous variables. $X_1$ and $X_2$ are plotted in a scatterplot and assume that the scatterplot is binned vertically into $b$ bins. \hh{XXX in the package, the binning is horizontal. }
\hh{XXX how are these bins motivated? what kind of example do you have in mind? }
In each vertical bin, a linear regression model is fitted and the regression coefficients. i.e.\ intercept and slope are estimated. The distance metric based on the regression coefficients is given as
 \begin{eqnarray}\nonumber
d^2_{\hbox{RG}}(X, Y) := \hbox{tr} (B(X) - B(Y))' (B(X) - B(Y))  \\
= \sum_{i=1}^b ((b_0(X))_i - (b_0(Y))_i)^2 + \sum_{i=1}^b ((b_1(X))_i - (b_1(Y))_i)^2\label{eq:drg}
\end{eqnarray}
\hh{XXX intercept and slope could be on very different scales.  On what kind of examples did you try this distance out? 
XXX I think we should try to scale X and Y to the same scale first. }
...
With larger number of bins (i.e. smaller bin sizes), the regression coefficients are affected by the skewness of the data. Variations might include using slope alone, or absolute value of slope. \hh{XXX are regression coefficients really affected by skewness or is it sparseness? What is the sample size of the examples that you tried?}

\item {\bf Distance based on separation between multiple groups (MS, AS, WL):}
\hh{XXX is the distance WL or CM? }
Let $X_1$ and $X_2$ be two continuous variable. Let $X_3$ be a categorical variable providing the groups associated with each variable. $X_1$ and $X_2$ are plotted in a scatterplot colored by the group variable $X_3$. The separation can be described in a number of different ways. Two versions are used in this paper. \hh{XXX two versions or three? } %Let us define, 
...
\item[(ii)] {\bf Average point separation:} let $s_{a}(.)$ be a vector of cluster wise average distances of all the points in the cluster to all points in all other $g-1$ clusters. The distance metric based on average separation is defined as
\[
...
\hh{XXX Why are there so many variations for the cluster distance? There are obvious variations for the other distances mentioned, too - it would be more consistent to just pick one distance and run with it.}

Figure~\ref{sep-dist} illustrates the difference between the different distance metrics separation. In practice, many possible metrics could be used to measure the separation, such as those readily available in the {\tt fpc} package  \citep{hennig:2010}.
...
\caption{\label{type1_minsep} Minimum Separation}
\includegraphics[width=\textwidth]{min-sep-example.pdf}
\end{subfigure}
\begin{subfigure}[t]{0.3\textwidth}
\caption{Average Separation\label{type_2_avesep}}
\includegraphics[width=\textwidth]{ave-sep-example.pdf}
\end{subfigure}
\begin{subfigure}[t]{0.3\textwidth}
\caption{Cluster Mean Separation\label{type_2_clsep}}
...
\hh{XXX still open questions after the first read-through: how are these distance metrics chosen, how well do they work in general situations? At the moment the distances seem like they are tailored for very specific examples in very specific designs.}
...
% \item Generate a null dataset from the true dataset by the null generating mechanism. Consider this null dataset as ``true'' dataset. From this ``true'' dataset, generate ($m$ - 2) null datasets and calculate the average distance between the ``true'' data and the null datasets. Hence obtain a distance corresponding to this ``true'' dataset. 

\item \hh{Generate a lineup of size $m$ using the null generating mechanism. 
Single out one of these nulls as the `data' plot, and calculate the distances as described in steps (1) and (2).  } Repeat this procedure $B$ times (where $B$ is large, usually on the order of 1,000 or 10,000).

\item Plot these $B$ distances to obtain the empirical distribution of the distance metric. Plot the distances for the true plot and the null plots to show the distances for the lineup \hh{under consideration}. 
\end{enumerate}

% \hh{XXX the paragraph below sounds like a re-do of the above algorithm, except that I just named the number of reps $B$. }
...
%The empirical distribution of the distance works as the $t$-distribution in the classical setting. In the classical setting, the test statistics follows a $t$-distribution under the null hypothesis. 
%Unlike the $t$-distribution in classical hypothesis tests, the empirical distribution in visual inference is generally skewed.

The observed test statistic is  compared to this distribution, as shown in Figure~\ref{compare}. 
%In visual inference, the mean distances of the null plots gives the empirical distribution. The mean distance of the true plot from the null plots in the lineup acts as the observed test statistic. 

%The mean distance between the true plot and the null plots in a lineup of size $m = 20$ is calculated by averaging over the distances between the true plot and each of the  $(m - 1)$ null plots. The mean distances for the $(m - 1)$ null plots in the lineup are calculated by taking the mean of the distances of the particular null plot and the other $(m - 2)$ null plots.

The distance measures for the true dataset and the null datasets are plotted on the empirical distribution. If the distance measure of the true plot is larger than any of the null plots, the lineup would be regarded as``easy''. Otherwise, it is a ``difficult'' lineup. 

The empirical distribution of the distance based on regression is shown in Figure~\ref{fig:distances}. To generate this distribution, $B = 1000$ simulation runs were done. 

Figure~\ref{dist_1} shows the lineup plot for $m = 20$ for testing whether there exists a significant linear relationship between $X_1$ and $X_2$. The 19 null plots are generated by fitting the null model and generating from the null model. Figure~\ref{dist_2} shows the general empirical distribution of distance measures based on the null model. For the particular lineup on the left, mean distances  are shown by overlaid line segments for the true plot (in orange) and the null plots (in black). The true plot is easily identifiable from the lineup (Figure~\ref{dist_1}, \hh{40 out of 45 observers identified the data}). This is backed by the regression based distance measure seen in Figure~\ref{dist_2}, as the orange line is on the extreme right tail compared to the black lines. 
...
\caption{\label{dist_2}Regression based distance metric.}
...
for (i in 1:nrow(metrics.dat)) {
  ...
  metrics.dat$reg.dist[i] <- reg_dist(P, PX)
  metrics.dat$reg.dist[i] <- reg_dist(P, PX, intercept=FALSE)
}
...
\hh{XXX the goal this needs to be stated more clearly. I am not quite sure at this point of why we are trying to get the distance measures to reflect the choice so closely. I think this is getting into fighting wind mills - we can't win this one. }

The question posed to observers plays an important role in the decision they make. A minor change in the question can change the response of the subject. For example,  if  subjects are asked to identify the plot  from on of the lineups in Figure~\ref{fig:sca}, in which there is the strongest positive relationship between the variables, plot \#2 exhibits the strongest signal.  If the question asks for the plot with the largest slope, plot \#7 should be the obvious pick. 
\hh{XXX again, why do we try to reflect a plot in a single number, when we know that we can't?}
A distance metric should also take into account the question of interest. But, in general, the question of interest is which plot among these is different.

\hh{XXX it matters what kind of comparisons those distance measures are supposed to allow. Internally, when the same design is used, the measures are consistent - and alow for a relative comparison: an easier lineup will show a bigger positive distance between the true plot and the nulls than a more difficult one. The distance measure does not necessarily allow for an absolute assessment of difficulty. When the comparison is additionally supposed to distinguish between designs showing the same data, the distance measures definitely need to take the graphical elements used into account. }

\section{Metric Evaluation} \label{sec:eval}
%
For a lineup of size $m = 20$, the average distance of the true plot from all null plots is compared 18 average distances between the null plots. This comparison can sometimes complicate things. A logical solution is to calculate a single statistic for each lineup. Such a statistic should be based on the mean distance of the true plot and the maximum of the mean distances for the null plots. Hence we define, 
\begin{enumerate}
\item {\bf Difference:} let $\bar{d}_.$ be the average difference of plot $.$ to all of the (other) null plots. We define the difference between the mean distance for the true plot and the maximum of the mean distances for the null plots as a measure of lineup difficulty, more specifically, 
\begin{equation}\label{eq:delta}
\delta(\ell) = \bar{d}_{\text{data}} - \max_j \left(\bar{d}_{\text{null}_j}\right)
\end{equation}
for $j = 1, \dots, (m  - 1)$ 
defines the lineup difference for lineup $\ell$.
 A positive difference  indicates that the mean distance of the true plot is larger than the maximum of the mean distances of the null plots. Hence the true plot is more extreme compared to the set of null plots in the lineup. A larger difference should make a data plot identification easier. 
...
The issue with this statistic is that $\delta$ indicates an ``easy" or ``difficult" lineup only on the basis of whether it is positive or negative, although it may be really close to 0.
\hh{XXX why should we base the decision on difficulty on the sign only? Shouldn't the absolute value of the difference also taken into account?}



However, this statistic does not imply how many null plots are more extreme than the true plot. So we define,
\item {\bf \hh{Number of Extreme Nulls}:}  the number of null plots which have larger mean distances than the mean distance of the true plot is noted. Mathematically, for lineup $\ell$, we define the $\gamma$-number as
\begin{equation}\label{eq:nullcount}
\gamma(\ell) = \sum_{j = 1}^{m - 1} 1\!\!\hbox{I}\left(\bar{d}_{\text{null}_j} > \bar{d}_{\text{data}}\right),
\end{equation}
...
% \end{cases}
% \end{equation}
$\gamma(.)$ takes integer values between 0 and $(m - 1)$. The higher this value, the more null plots are more extreme than the true plot and hence the data plot is harder to identify from the lineup.
\end{enumerate}

\hh{XXX why don't you also use the distribution of the null distances that you created? That should give an empirical $p$-value for the data plot - it does not, however, take the dependency on the nulls into account.}
...
The performance of the distance metrics was evaluated by comparing  distances to observers' responses. A number of experiments were run using the MTurk service \citep{turk}. Subjects were recruited through this service and were shown a sequence of lineups. In each experiment, they were asked specific questions. Their responses were recorded along with other demographic informations. Details on the design of experiments can be found in \cite{majumder:2011} and \cite{roychowdhury:2013}. 

\hh{XXX above paragraph could be tightened up a bit}
\hh{XXX still missing - what is the plan for this section?}
\hh{XXX plan seems to be: discuss evaluation metrics at the example of three turk studies. }

\subsection{Turk Experiment 1 -- Side by Side Boxplots}\label{sec:turk1}

\hh{XXX we've had this information earlier}
In this experiment, all the lineups generated had a side by side boxplot as the test statistic. Assuming that the null hypothesis is true, the null plots were generated by assuming that there is no difference between the two distributions.
\hh{XXX too vague}
The subjects were shown a few lineups and were asked to identify the plot which has the largest vertical difference between group 1 and group 2. 
...
\caption{\label{fig:turkcomp-1}\hh{Scatterplots of detection rate versus $\delta$-difference}}
...
\begin{subfigure}[t]{\textwidth}
\centering
\caption{\hh{Scatterplots of detection rate versus $\gamma$-number of extreme nulls}}
...
In Figure~\ref{turk1comp},  detection rate is plotted against $\delta$-difference. The  vertical line highlights a  difference of 0 indicating that the mean distance of the true plot is equal to the maximum of the mean distance of the null plots i.e.\ the mean distance of the true plot is equal to at least one of the mean distance of the null plots. As the $\delta$-difference increases, detection rate generally increases. %So the subjects do better in the easier lineups than the hard ones. 
\hh{The thick lines in Figure~\ref{fig:turkcomp-1} are fits  from  logistic regression models using a quadratic effect for distance.
The fitted lines come very close to the non-parametric smooth shown by the thin line. Qualitatively, both boxplot based distance and binned distance show very similar fits for detection rate. Based on the logistic regressions, boxplot distance is fitting detection rate a bit better (AIC: \Sexpr{round(AIC(dbx),1)}) than binned distance  (AIC: \Sexpr{round(AIC(dbn),1)}). }

 Figure~\ref{turk1comp} also shows the relation between detection rate and the number of null plots larger than the true plot. As the number of plots that are more extreme than the data plot increases,  subjects find it more and more  difficult to pick the data plot. It is interesting to see that subjects are able to  pick the data plot even if there are  one or two more extreme null plots. 


Though  distance based on  boxplots works better a bit better, the binned distance does a decent job in this case. According to the binned distance, there are a few lineups with a negative difference, in which nevertheless the data plot is identified in more than 60\% of all evaluations (see also Figure~\ref{turk1comp}). It should be noted that the binned distance does not take any graphical elements of the plot (such as the box or whiskers) into account but calculates distance solely based on the data. So outliers may have a huge effect on the binned distance but might not affect the distance based on the boxplots. 
\hh{XXX the following sentence is a bit hard to swallow. XXX the case against one single distance needs to be built up more slowly. }
Hence it is advisable to use a distance based on the graphical elements since that is exactly what the subjects look at in the lineup.

\hh{In order to investigate on what participants base their choice, we are going to have a closer look at a lineup. }
\hh{From Figure~\ref{turk1comp} we see that for some of the lineups the detection rate is higher than its $\delta$-difference would suggest.} One such lineup is marked using a triangle in Figure~\ref{turk1comp}. %It would be interesting to look into the lineup closely to identify what made the people pick the actual plot as different. 
Figure~\ref{turk1-exp} shows the lineup and the distribution of the distance metrics for a closer look at what might observers lead to pick the data plot as different. \hh{The grey numbers at the bottom right of each of the plots in the lineup gives an overview of which plots participants picked in 168 evaluations of the lineup.} 

\begin{figure}[hbtp]
\centering
\begin{subfigure}[t]{\textwidth}
\centering
\caption{\label{fig:turk1-exp-1}Lineup of side-by-side boxplots. The numbers at the bottom right are the number of times each of the plots was chosen by observers.}
...
\end{subfigure}
	...
	\vspace{-.1in}
\caption{ Illustration of the behavior of the different distance metrics. The lineup is shown in (a) and the distributions of different distance metrics based on this lineup is shown in the other plots: boxplot based distance in (b), binned distance with 2 and 8 bins on x and y axis in (c) and binned distance with 2 bins in both axes in (d). The lineup corresponds to the point marked with a triangle in difference vs. detection rate plot in Figure~\ref{turk1comp}. }
\label{turk1-exp}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
%\afterpage{\clearpage}
...
\hh{The observed data plot is Plot \#20, which has been picked most often by participants, but there are other plots that are being chosen quite frequently. Plot \#16 is the plot with the  largest boxplot distance. This is reflected by the large number of times this plot has been singled out by observers. 
...
The time subjects  take to respond  is another measure that can be used to evaluate  difficulty of lineups. 
...
\hh{Both $\delta$-difference exhibit a similar pattern: the time to respond peaks at a $\delta$-difference of zero. This indicates, that the situation where two or more plots exhibits similarly extreme features is the most difficult for observers to judge. With negative $\delta$-differences at least one null plot is more extreme, and observers seem to be able to make their choice quickly (probably for the extreme null plot).}
...
\hh{Note that taking the log of time to respond is an alternative to using the median. Qualitatively, the relationships to distance metrics are very similar for median and log time taken, but median time is much easier to interpret. }
...
\hh{We need a few more specific details: }
The null hypothesis underlying the lineups for this study is that there is no relationship between two normal random variables. As a test statistic, a scatterplot with an overlaid regression line is used. 
Lineups were created for data plots exhibiting a linear relationship between the two variables for a range of correlation values \citep{majumder:2011}. Figures~\ref{lineup-example} and~\ref{fig:distances} show examples of the type of  lineup used in the study.  Subjects recruited from MTurk  were shown a set of ten lineups each and asked to identify the plot with the steepest slope. 
...
The distances between the plots in this experiment were computed using both the regression based distance  ($d_{RG}$) and the binned distances ($d_{BN}$) with a small number of bins. For each lineup the proportion of data identifications was calculated from participants' responsea and plotted against $\delta$ and $\gamma$.

\hh{XXX}
Figure~\ref{turk2comp} shows the results for the distance based on the regression line and the binned distance against $\delta$.
...
t2drg <- glm(cbind(correct, n-correct)~diff.reg, data=prop.dist, family=binomial())

predrg <- data.frame(diff.reg=seq(-20,40, by=1))
...
Figure~\ref{turk2comp-1} shows a scatterplot of detection rate against $\delta$-difference. The vertical line represents difference equal to zero. It can be seen that as the difference increases, the detection rate increases. So the subjects do better in the easier lineups than the hard ones. The distance based on regression works well in capturing the complexity of the lineups. The difference is positive for lineups with large differences and negative for lineups with small difference. A few lineups have difference close to zero for which the detection rate is close to 50\%. The thick smooth line shows the fit of a logistic regression of detection rate in $\delta$-difference (AIC: \Sexpr{round(AIC(t2drg),1)}).
...
Figure~\ref{turk2comp} also shows the detection rate against the $\gamma$-number of extreme null plots. As the number increases,  detection rate decreases on average --- as there are more extreme null plots compared to the  plot, it becomes more difficult to identify the data plot. For a few lineups, almost all the subjects identify the observed plot although there is one more extreme null plot. Though from Figure~\ref{turk2comp}, it can be seen that the difference is marginal in most cases. 
...
Figure~\ref{turk2-mtime} shows the relationship between the median time taken to respond and $\delta$-difference for both the distances. It can be clearly seen that there is a strong negative association: as $\delta$-difference increases, the subjects take less time to respond. \hh{Similar to the previous example of section~\ref{sec:turk1} time to respond peaks at a $\delta$-difference close to zero is lower for larger negative $\delta$-difference based on regression distance.}
...
Although the regression based distance seems to efficiently identify the quality of the lineup, there is one lineup (marked by a solid triangle in Figure~\ref{turk2comp}) \hh{with} a negative $\delta$-difference which was nevertheless identified by observers reasonably successfully.  Figure~\ref{turk2-exp} shows the lineup and the corresponding distributions of distance metrics. 
...
% \begin{subfigure}[t]{0.3\textwidth}
%\includegraphics[width=\textwidth]{distribution-reg-no-int-dist-exp2.pdf}
\caption{Regression w/o intercept}
\vspace{-.1in}
<<fig.height = 4.125, fig.width = 4.5, echo=FALSE, out.width='\\textwidth'>>=
yregx <- max(density(dframe$drgx)$y)
ggplot()  + 
  geom_density(data = dframe, aes(x = drgx), fill = "grey80", col = "grey80") +
  geom_segment(data = subset(dd3, len != 19), 
               aes(x = brgx, xend = brgx, y = rep(0.02*yregx,19), 
                   yend = rep(0.1*yregx,19)), 
               size=1, alpha = I(0.7)) +
  geom_segment(data = subset(dd3, len == 19), 
               aes(x= brgx, xend = brgx, y=0.02*yregx, yend = 0.2*yregx), 
               colour="darkorange", size=1) +
  xlab("Regression Distance\n(no intercept)") + ylab("") + 
  geom_text(data = subset(dd3, len != 19), size = 2.5, 
            aes(x = brgx, label = pos.1, y = - 0.04*yregx + (pos.1 %% 4)/400)) + 
  geom_text(data = subset(dd3, len ==19), y = 0.23*yregx, size = 3, 
            aes(x = brgx, label = pos.1), colour="darkorange") + 
  ylim(c(- 0.04*yregx, yregx + 0.1*yregx))+
  theme(plot.margin=unit(c(0,0,0,0), unit="line"))
@
\end{subfigure}
...
\caption{Illustration of the behavior of different distance metrics. The lineup is shown in (a) and the distributions of different distance metrics using this lineup are shown in plots (b)--(e): regression based distances in (b) and (c), binned distance with 2 bins on each axes in (d), binned distance with 8 and 2 bins in x and y axis respectively in (e). In (f), the distribution of the conventional $p$-values are plotted with $p$-values for the lineups marked on the distribution. The lineup corresponds to the point marked with a triangle in difference vs. detection rate plot in Figure~\ref{turk2comp}.}
\label{turk2-exp}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
%\afterpage{\clearpage}
...
\hh{For the data plot (in panel \#10) the conventional $p$-value for testing the slope equal to zero is 0.085. However, the signal in the plot is strong enough, to make  around 28\% of all subjects pick this plot.}
...
\hh{The number of picks lines up best with the difference measure based on the slope (regression without intercept). Plots \# 10, 12, 11, 9, and 7 all have a relatively steep slope, and are also the plots that were picked the most often.  }

\hh{XXX something at the end about the graphical elements? How about this:}

\hh{Again, a distance measure derived directly from one of the graphical elements in the plot leads to the best assessment of the choices made by human evaluators.}


\subsection{Turk Experiment 7 -- Large $p$, Small $n$ Data}

The motivation behind this experiment is to study the effect of \hh{high dimensionality} on separability in data. Scenarios with pure noise and some real separation in two or three groups were investigated. %Data was simulated with different dimensions and fixed sample size. Data was divided into two or three groups. 
...
%\includegraphics[width=.65\textwidth]{large-p-trim-bin.pdf}
<<t7-lp, echo=FALSE, fig.width=6, fig.height=6, out.width='.8\\textwidth'>>=
...
<<largep-delta, echo=FALSE, dependson='t7-distr', out.width='\\textwidth', fig.width=8.5, fig.height=3.75>>=
dbn <- glm(cbind(correct,n-correct)~poly(diff.dbn,1), data=prop.dist, family=binomial())
...
dpred2 <- data.frame(diff.dms=seq(-1, 2, length=100))
...
# 
dpred <- rbind(dpred1, dpred2, dpred3)


### Facetted Plots
prop.diff <- subset(prop.dist, select = c(pic_name, diff.dms, diff.das, diff.dbn,  prop, m.time, med.time, shape))
prop.diff.m <- melt(prop.diff, id.vars = c("pic_name", "prop", "m.time", "med.time", "shape"))
...
<<largep-gamma, echo=FALSE, dependson='t7-distr', out.width='\\textwidth', fig.width=8.5, fig.height=3.75>>=
### Facetted Plots
prop.grtr <- subset(prop.dist, select = c(pic_name, grtr.dms, grtr.das, grtr.dbn,  prop, m.time, med.time, shape))
prop.grtr.m <- melt(prop.grtr, id.vars = c("pic_name", "prop", "m.time", "med.time", "shape"))

levels(prop.grtr.m$variable) <- c("Minimum Separation", "Average Separation","Binned Distance")
...
ggplot(aes(x=value, y=med.time), data=prop.diff.m) + 
  geom_vline(xintercept = 0, col = "grey50") +
  geom_point(aes(shape=factor(shape)), size=3) +
  xlab(expression(paste(delta,"-Difference",sep=""))) + ylab("Median time to respond (in seconds)") +
  facet_wrap( ~ variable, scales = "free_x") + 
  theme(legend.position = "none") + 
...
@
	...
	\vspace{-.1in}
\caption{\hh{XXX changed mean time to median time} Plot showing the mean time to respond by the subjects against the difference based on the minimum separation distance, average separation and binned distance. The vertical line represents the difference equal to 0 when there is at least one null plot similar to the observed plot. The mean time decreases as the difference increases.  }
\label{lp-mtime}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
%\afterpage{\clearpage}
...
\caption{Lineup of scatterplots of three groups. Which plot shows the best separation?}
...
\end{subfigure}
	...
	\vspace{-.1in}
\caption{Illustration of the behavior of different distance metrics. The lineup is shown in (a) and the distributions of different distance metrics are shown in the other plots:  binned distance with 6 and 4 bins in x and y axis respectively in (b), distance based on average separation in (c) and distance based on minimum separation in (d). \hh{XXX I'm not able to re-create the code for the distance densities. It seems that the code in metrics-papers.R is based on just 15 reps - I stopped the run after several hours. Is there a faster way for creating (approximate) null plots?}}
\label{lp-exp}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
%\afterpage{\clearpage}

Figure~\ref{lp-exp} shows the lineup in a high dimension, low sample size setting. The number of dimensions used is 100 and two of the dimensions have some separation. Plot 20 shows the two-dimensional projections of the original data.  Since the true plot has real separation, it is expected that the subjects would be able to identify the plot. The distance based on average separation yields a negative difference showing that the lineup is difficult, while the distance based on minimum separation yields a positive difference. The distance metrics identifies different characteristics in a plot. The average separation looks at the average of the distances of the points in a cluster to the points in other clusters. The presence of an outlier point in the opposite side of the other clusters affects this distance considerably. On the other hand, the minimum separation looks at the minimum of the distances. Hence it is not affected by the outlier point. 
...
This work was funded by National Science Foundation grant DMS 1007697. All studies were conducted with approval from the Institutional Review Board IRB 10-347. All plots are done with the {\tt ggplot2} \citep{hadley:2009} package in R.
