...
dd3 <- metrics.dat %>% group_by(pos.1) %>% summarize(
  reg.mean = mean(reg.dist), len=n())

@

<<dens-reg-dist, dependson='turk2-lp', echo=FALSE, fig.width=4.5, fig.height=4, out.width='\\textwidth'>>=
dframe <- data.frame(reg=dfreg)
ymax <- max(density(dframe$reg)$y)
ggplot()  + 
  geom_density(data = dframe, aes(x = reg), 
               fill = "grey80", col = "grey80" ) + 
  geom_segment(data = subset(dd3, len != 19), 
               aes(x = reg.mean, xend = reg.mean, 
                   y = rep(0.02*ymax,19), yend = rep(0.1*ymax,19)), 
               size=1, alpha = I(0.7)) + 
  geom_segment(data = subset(dd3, len == 19), 
               aes(x= reg.mean, xend = reg.mean, 
                   y=0.02*ymax, yend = 0.2*ymax), colour="darkorange", size=1)  +
...
\centering
\begin{subfigure}[b]{0.55\textwidth}
\caption{Lineup: which of these pairs of boxplots shows the biggest vertical difference?}
...
\end{subfigure}
\begin{subfigure}[b]{0.44\textwidth}
\caption{Boxplot based distance}
