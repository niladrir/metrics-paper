...
The question posed to observers plays an important role in the decision they make. A minor change in the question can change the response of the subject. For example,  if  subjects are asked to identify the plot  form Figure~\ref{box-1}, in which the green group has a larger vertical difference than the red group, plot \#6 exhibits the strongest signal.  If the question asks for the plot with the largest vertical difference between the two groups, plot \#15 shows the largest one. 
