...
\item {\bf \hh{Number of Extreme Nulls}:}  the number of null plots which have larger mean distances than the mean distance of the true plot is noted. Mathematically, for lineup $\ell$, we define 
...
\caption{\hh{XXX Smooth lines come from logistic regression models using a quadratic effect for distance. The fit is not terrific, but not too shabby. What do you think? AIC/anova of the binned  distance is a smidgen better than for the boxplot distance. XXX update: with the fixed metrics, boxplot distance is actually a little better.}}
...
<<gamma-turk1, dependson='dist-box-bin',fig.width=8.5, fig.height=4.5, out.width='0.7\\textwidth', warning=FALSE>>=
