...
\begin{enumerate}
\item[(i)]  $s_{m}(.)$ be a vector of cluster wise minimum distance between a point in the cluster to the points in other clusters for $g$ clusters. The distance metric based on separation is defined as
\[
d^2_{\hbox{MS}}(X, Y):= ||s_m(X) - s_m(Y)||^2 = \sum_{i = 1}^g ((s_m(X))_i - (s_m(Y))_i)^2
\]
\hh{XXX I don't get the math yet}

\item[(ii)]  $s_{a}(.)$ be a vector of cluster wise average distances of all the points in the cluster to all points of all other $g-1$ clusters. The distance metric based on separation is defined as
\[
d^2_{\hbox{AS}}(X, Y):= ||s_a(X) - s_g(Y)||^2 = \sum_{i = 1}^g ((s_a(X))_i - (s_a(Y))_i)^2
\]

\item[(iii)] Let $m_i(.) = (\bar{X_1}^i, \bar{X_2}^i)$ be the cluster mean for the $i$-th cluster, $i = 1, 2, \dots, g$. The distance metric based on the cluster means is given by
\[
