...
Figure~\ref{turk1-exp} shows the lineup and the distribution of the distance metrics for a closer look at what might observers lead to pick the data plot as different. 
\hh{XXX What did observers pick??}
...
\begin{subfigure}[t]{\textwidth}
\centering
\caption{Lineup of side-by-side boxplots}
...
\hh{XXX the discussion should be linked more to the metrics.}
