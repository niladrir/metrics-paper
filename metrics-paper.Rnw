...
\hh{XXX I think there needs to be a bit of discussion, of why $\delta$-difference is used rather than a comparison of the empirical $p$-values. $p$-values would be comparable across different distance metrics, and it would be logical to use them, as we just identified the distribution of the whole thing.}
...
II &  Scatter plot & \begin{minipage}[c]{2cm}  \begin{center} \scalebox{0.3}{\includegraphics{stat_beta_k1.png}} \end{center} \end{minipage} & Of the scatter plots below which one shows data that has steepest slope? \citep[see also][]{majumder:2011}\\
III & Group separation & \begin{minipage}[c]{2cm} \begin{center}  \scalebox{0.4}{\includegraphics{stat_separation.pdf}} \end{center} \end{minipage} & Which of these plots has the most separation between the coloured groups?  \citep[see also][]{roychowdhury:2013}\\
\hline 
\end{tabular}
