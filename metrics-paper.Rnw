...
\hh{XXX this will need some adjustement to reflect the restructuring XXX}
...
\hh{XXX why should we base the decision on difficulty on the sign only? Shouldn't the absolute value of the difference also be taken into account?}



However, this statistic does not imply how many null plots are more extreme than the true plot. So we define,
\item {\bf $\gamma$-\hh{Number of Extreme Nulls}:}  the number of null plots which have larger mean distances than the mean distance of the true plot is noted. Mathematically, for lineup $\ell$, we define the $\gamma$-number as
\begin{equation}\label{eq:nullcount}
\gamma(\ell) = \sum_{j = 1}^{m - 1} \mathds{1}\left(\bar{d}_{\text{null}_j} > \bar{d}_{\text{data}}\right),
\end{equation}
...
Some experiments are used for evaluating the power of visual inference against that of classical tests \citep{majumder:2011}, \hh{some for comparison of different designs \citep{hofmann2012graphical, loy:2015}, or for targeted conclusions based on visual inference in situations where traditional tests do not perform well \citep{tengfei:2013}.} In each study, subjects were recruited through the MTurk service and were shown a set of lineups.
%and the others using the protocol for different purposes. 
 For evaluating the distance metrics we used the data collected on experiments~1, 2, and~7 described in Table~\ref{tbl:visual_stat}.
...
between group A and B? \citep[see also][]{majumder:2011} \\
2 &  Scatter plot & \begin{minipage}[c]{2cm}  \begin{center} \scalebox{0.3}{\includegraphics{stat_beta_k1.png}} \end{center} \end{minipage} & Of the scatter plots below which one shows data that has steepest slope? \citep[see also][]{majumder:2011}\\
  7 & Group separation & \begin{minipage}[c]{2cm} \begin{center}  \scalebox{0.4}{\includegraphics{stat_separation.pdf}} \end{center} \end{minipage} & Which of these plots has the most separation between the coloured groups?  \citep[see also][]{roychowdhury:2013}\\
\hline 
\end{tabular}
\label{tbl:visual_stat} 
\end{table*} 

We evaluated the performance of the distance metrics in each of the experiments by comparing  distances to responses from observers. 
...
\hh{The question of interest in this study is to investigate the power of visual inference in the classical situation of assessing the significance of a co-variate $X$ in a linear model. For the first study, the assumption is that the covariate is discrete (with two levels), in the study Turk Experiment 2 (see section~\ref{sec:turk2}), we assume the covariate to be continuous. 
...
Each of the subjects recruited through the MTurk service, was asked to evaluate ten lineups, and to identify in each the plot that exhibits the largest vertical difference between groups A and B. 
...
\newpage
\subsection{Turk Experiment 2 -- Scatterplots with an Overlaid Regression Line}\label{sec:turk1}
