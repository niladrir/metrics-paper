...
prop.dist <- dd %>% group_by(pic_name) %>%
  summarize(diff.box = box.mean[len == 19] - max(box.mean[len == 18]), grtr.box = sum(box.mean[len == 18] > box.mean[len == 19]) , diff.bin = bin.mean[len == 19] - max(bin.mean[len == 18]), grtr.bin = sum(bin.mean[len == 18] > bin.mean[len == 19]) , prop = mean(prop), m.time = mean(m.time), n=mean(n), correct=mean(correct))
...
### Faceting
dbn <- glm(cbind(correct,n-correct)~diff.bin, data=prop.dist, family=binomial())
dbx <- glm(cbind(correct,n-correct)~diff.box, data=prop.dist, family=binomial())
prop.dist$dbn <- predict(dbn, type="response")
prop.dist$dbx <- predict(dbx, type="response")
...
ggplot(data = prop.diff.m) + 
  ...
  geom_point(aes(x=value, y=prop, shape = factor(special)), 
             size = 3, alpha=0.8) + 
  ylim(c(0, 1)) + xlab("Difference")+ ylab("Detection Rate") + 
