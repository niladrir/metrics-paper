...
Let $X_1$ and $X_2$ be two continuous variables. $X_1$ and $X_2$ are plotted in a scatterplot and assume that the scatterplot is binned vertically into $b$ bins.

\hh{XXX in the package, the binning is horizontal. }
...
\[
d^2_{AS}(X, Y):= ||s_a(X) - s_g(Y)||^2 = \sum_{i = 1}^g ((s_a(X))_i - (s_a(Y))_i)^2
\]
...
\hh{XXX lead into the next section}
...
\item Plot these $B$ distances to obtain the empirical distribution of the distance metric. Plot the distances for the true plot and the null plots to show the distances for the lineup under consideration. 
\end{enumerate}
...
\caption{\label{dist_2}Regression based distance metrics: unscaled at the top, scaled at the bottom. \hh{XXX my suggestion would be to switch to the scaled regression based distance instead of the unscaled one.} \hh{XXX I did the switch; the distribution below is the only one, that is unscaled now}}
