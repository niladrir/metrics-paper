...
prop.diff <- data.frame(prop.diff)
prop.diff.m <- melt(prop.diff, measure.vars = c("diff.dms", "diff.das", "diff.dbn")) #, "diff.ddunn"))
...
### Facetted Plots
prop.grtr <- subset(prop.dist, select = c(pic_name, grtr.dms, grtr.das, grtr.dbn,  prop, #grtr.ddunn, 
