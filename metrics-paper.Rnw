...
Following up on choices, observers are asked to describe their reasoning. These reasons are used to obtain more information about the rejection: was it some nonlinear dependency, an outlier, clustering, that triggered the detection of the data plot? Good distance metrics may also help relate the descriptive words used to mathematically defined features. 

The article is organized as follows. ection~\ref{sec:null} discusses the null generating mechanisms. ection~\ref{sec:meas} defines the distance measures and discusses the choice of the measures. The distribution of the distance measures are studied in ection~\ref{sec:distri}. ection~\ref{sec:plot_type} describes the effect of the plot type and the question of interest on the distance measure while ection~\ref{sec:eval} talks about the distance evaluations. In ection~\ref{sec:nbin}, the methods to select the number of bins for the binned distance is described. ection~\ref{sec:results} presents a comparison of the distance measures to the performance of human subjects in several experiments conducted by Amazon's Mechanical Turk.
...
%In this paper we presents results of a human-subject study assessing the performance of the different distance matrices. ection~\ref{sec:visual_test} describes the basic idea of the visual inference and also gives the definition of the different distance measures. ection~\ref{sec:asso} and ection~\ref{user.distance} applies these ideas and uses the definitions to calculate the different distance measures. In ection~\ref{results} and ection~\ref{sec:conclusions} we outline the setup and present results.
...
using Amazon's Mechanical Turk~\citep{turk} (MTurk), with some used for evaluating the power of visual inference against that of classical tests, \hh{some for comparison of different designs \cite{hofmann2012graphical, loy:2015}, or for targeted conclusions based on visual inference \cite{tengfei:2013}.}
