...
\hh{XXX I'm not terribly happy with the next paragraph - it  might be because I don't quite see the storyline that it is supposed to fit into. One suggestion for the story would be `ideally, while distances based on graphical elements do better, binned distance would be a general approach independent of plot type and specific question, that does relatively well. Unfortunately, its limitations are quite restrictive. For turk1 binned distance performs almost as well as boxplot based distance. turk2 sows that binned distance is not able to deal well with large variability. The number of bins in X and Y direction is affecting the outcome a lot.' }
...
<<lp-exc, echo=FALSE, fig.width=6, fig.height=6, out.width='\\textwidth'>>=
