...
\begin{subfigure}[t]{\textwidth}
\centering
\caption{\label{turk2comp-1}$\delta$-difference \hh{XXX problem with binned distance - will re-calculate turk2-metrics next}}
...
qplot(value, prop, data = prop.diff.m, geom = "point", size = I(3), xlab = "Difference", ylab = "Detection Rate", shape = factor(special)) + 
