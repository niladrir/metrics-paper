...
\hh{The thick lines in Figure~\ref{fig:turkcomp-1} are fits  from  logistic regression models using a quadratic effect for each of the two distances.
The fitted lines come very close to the non-paramteric smooth shown by the thin line. Qualitatively, both boxplot based distance and binned distance show very similar fits for detection rate. Based on the logistic regressions, boxplot distance is fitting detection rate a bit better (AIC: \Sexpr{round(AIC(dbx),1)}) than binned distance  (AIC: \Sexpr{round(AIC(dbn),1)}). }
...
dframe <- data.frame(t(replicate(5000, {
	samp.dat <- data.frame(group = obs.dat$group, 
...
\caption{\small Binned ($p=2, q=8$) distance}
...
\caption{\small Binned ($p=2, q=2$) distance}
