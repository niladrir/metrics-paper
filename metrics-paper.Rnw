...
= \sum_{i=1}^b ((b_0(X))_i - (b_0(Y))_i)^2 + \sum_{i=1}^b ((b_1(X))_i - (b_1(Y))_i)^2
\end{eqnarray*}
...
\hh{XXX I'm not teribly happy with the next paragraph - it  might be because I don't quite see the storyline that it is supposed to fit into. One suggestion for the story would be `while distances based on graphical elements do better, binned distance is a general approach independent of plot type and specific question, that does relativley well. But we have to be aware of its limitations' }
...
%}
	\vspace{-.1in}
\caption{Comparison of distance metrics for side-by-side boxplots. Mean time to respond is plotted against the difference based on the boxplot and binned distance. The vertical line represents the difference equal to 0 when there is at least one null plot similar to the observed plot. The mean time taken decreases with the difference. The triangle represents a lineup which is examined further in Figure~\ref{turk1-exp}. }
\label{turk1-mtime}
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\end{figure}
\afterpage{\clearpage}

We noticed in Figure~\ref{turk1comp} that for some of the lineups, the detection rate is higher than $\delta$-difference would suggest. One such lineup is marked using a triangle in Figure~\ref{turk1comp}. %It would be interesting to look into the lineup closely to identify what made the people pick the actual plot as different. 
...
\begin{subfigure}[t]{\textwidth}
\centering
\caption{\label{turk2comp-1}$\delta$-difference \hh{XXX problem with binned distance - will re-calculate turk2-metrics next} \hh{XXX model for regression based distance works extremely well}}
\includegraphics[width=0.7\textwidth]{turk2-prop-reg-bin.pdf}
<<delta-turk2, echo=FALSE, fig.width=8.5, fig.height=4.5, out.width='0.7\\textwidth'>>=
...
predbn <- data.frame(diff.bin=seq(-11,5, by=0.25))
...
ggplot(data=prop.diff.m) + 
  ...
  theme(legend.position = "none") + 
  geom_line(aes(value, pred, group=variable), colour="steelblue", size=1, data=t2pred)
...
%}
	\vspace{-.1in}
\caption{Comparison of distance metrics for scatterplots with a regression line over laid. Mean time to respond is plotted against the difference based on the regression based and binned distance. The vertical line represents the difference equal to 0 when there is at least one null plot similar to the observed plot. The mean time taken decreases with the difference. The triangle represents a lineup which is examined in Figure~\ref{turk2-exp}.   }
%{Caption of subfigures \subref{fig:subfig1}, \subref{fig:subfig2} and \subref{fig:subfig3}}
\label{turk2-mtime}
\end{figure}
\afterpage{\clearpage}
