...
<<dist-box-bin, echo=FALSE, fig.width=8.5, fig.height=4.5, out.width='\\textwidth'>>=
...
\includegraphics[width=\textwidth]{turk1-grtr-box-bin.pdf}
\end{subfigure}
%\subfigure[]{
...
But when the difference is above 0, the median time to respond decreases rapidly as $\delta$-difference increases. Hence, subjects are able to make their choice of a plot more quickly if the true plot is extreme compared to the null plots.

\hh{XXX I don't think that the following statement is true -- there is no reason, why boxplot distance and binned distance are on the same scale. }
...
\begin{subfigure}[t]{0.49\textwidth}
\includegraphics[width=\textwidth]{turk2-prop-reg-bin.pdf}
\end{subfigure}
\begin{subfigure}[t]{0.49\textwidth}
\includegraphics[width=\textwidth]{turk2-grtr-reg-bin.pdf}
\end{subfigure}
%\subfigure[]{
