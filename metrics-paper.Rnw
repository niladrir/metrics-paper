...
\if0\blind
{
  \title{\bf Measuring Lineup Difficulty By Matching Distance Metrics with Subject Choices with Crowd-Sourced Data}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf  Measuring Lineup Difficulty By Matching Distance Metrics with Subject Choices with Crowd-Sourced Data}
\end{center}
  \medskip
} \fi
...
res.exp1 <- read.csv("data/raw_data_turk1.csv")
#lpsub <- unique(subset(res.exp1, pic_name=="plot_turk1_100_8_12_2.png")[,c("response", "response_no")]) \dc{why do we need unique, messes up calculations}
...
### Number of picks by observers
res.exp2 <- read.csv("data/raw_data_turk2.csv")
#lpsub <- unique(subset(res.exp2, pic_name=="plot_turk2_100_350_12_3.png")[,c("response", "response_no")]) \dc{why do we need unique, messes up computation}
...
\section{Conclusion}
Distance metrics are compared to the response of human subjects on lineups. What we see in each of the experiments is that the general approach of the binned distance, while mostly performing decently, is usually out-done by a distance that is more tailored to the question of interest or takes the graphical elements into account. These were a special collection of experiments where the task was very focused, but in practice the question is generic and we would expect the binned distance to be most broadly applicable. 

The distance metrics clearly can be used to help in future experiments with lineups, to roughly divide them into three groups, easy, moderate, difficult. Subjects are typically given a block of ten lineups to evaluate. Having the difficulty rating will help provide a reasonable set for each subject. 

It is also clear that some distance metrics better match with subjects' choices. Boxplot du=istance better matched subject choices in experiment I. Regression based distance matched better than binned distance for the subject choices in experiment II. By the $\gamma$-number minimum separation and average separation matched better than the other distance metrics for experiment III. 

None of the distance metrics perfectly matches subject choices. This suggests that there is a lot of scope for exploring new ways to numerically characterize structure in plots. The metrics we
have described still are based on the data, rather than the graphical elements in the plot. Utilizing the graphical elements, e.g. length of line relative the plot size to build metrics might provide a close fit, and also help to understand more precisely what viewers are visually responding to in the plot.
...
\item \dc{[Experiments:] Add the link to the experiments web site}  
