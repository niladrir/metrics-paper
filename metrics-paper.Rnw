...
$$\delta(\ell) = \bar{d}_{\hbox{true}} - \max_j \bar{d}_{\hbox{null}_j}$$
for $j = 1, \dots, (m  - 1)$ 
...
\item {\bf Larger than the true plot:} \hh{XXX that might need a simpler name} the number of null plots which have larger mean distances than the mean distance of the true plot is noted. Mathematically, for lineup $\ell$, we define 
\begin{equation}\label{eq:nullcount}
\gamma (\ell) = \sum_{j = 1}^{m - 1} 1\!|_{\bar{d}_{\hbox{null}_j} > \bar{d}_{\hbox{true}}}.
\end{equation}
% where 
% \begin{equation}
% a_j =
...
\caption{\hh{XXX Smooth lines come from logistic regression models using a quadratic effect for distance. The fit is not terrific, but not too shabby. What do you think? AIC/anova of the binned  distance is a smidgen better than for the boxplot distance.}}
<<dist-box-bin, echo=FALSE, fig.width=8.5, fig.height=4.5, out.width='0.7\\textwidth'>>=
...
ggplot(data = prop.diff.m) + 
  ...
  geom_point(aes(x=value, y=prop, shape = factor(special)), 
             size = 3, alpha=0.8) + 
  ylim(c(0, 1)) + xlab(expression(paste(delta,"-Difference", sep="")))+ ylab("Detection Rate") + 
...
\begin{subfigure}[t]{\textwidth}
\centering
\caption{\hh{still the old picture}}
...
\hh{XXX on what measure is the following statement based? AIC or anova shows that the binned distance is doing (a little bit) better. }
...
\hh{XXX wouldn't median time be better? XXX update: it is median time. 
Aside from that, we've been usually using the log of time taken - assuming that time taken is not that far from a Gamma distribution. XXX checked on log versus median, and the relationship to difference is qualitatively not that different. Median time is much easier to interpret than log times.  }
The time taken to respond by the subjects is another measure of difficulty of the lineups. Due to the presence of some huge outliers, median time taken by the subjects for each lineup is used and plotted against  $\delta$-difference for both the distance measures. Figure~\ref{turk1-mtime} shows the plots.
