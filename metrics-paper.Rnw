...
qplot(X, value, data=mlps, alpha=I(0.5), facets=~variable, size=I(2.5)) + 
  ...
  theme_lineup + xlab("X1") + ylab("X2") + theme(aspect.ratio=1)
@
...
\begin{table*}[hbtp] 
\centering 
\caption{Overview of the different Turk experiments, from which data was included to investigate distance metrics and how subjects read the plots. } 
...
By calculating the ``distance'' between plots we may be able to determine if a lineup should be easy -- f ithe actual data plot is detectably different from the null plots -- and also to better understand what aspect of the plot people use to make their choice. It is not an easy task to measure the difference between plots. Here we examine several possibilities. 
