...
\hh{to allow for a description of a piece-wise linear relationship between the variables.}
...
prop.diff <- data.frame(prop.diff)
prop.diff.m <- melt(prop.diff, measure.vars = c("diff.dms", "diff.das", "diff.dbn"))

levels(prop.diff.m$variable) <- c("Minimum Separation", "Average Separation","Binned Distance")
prop.diff.m$variable <- factor(prop.diff.m$variable, levels = c("Minimum Separation", "Average Separation","Binned Distance"))
...
prop.grtr.m$variable <- factor(prop.grtr.m$variable, levels = c("Minimum Separation", "Average Separation","Binned Distance"))
