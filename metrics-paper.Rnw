...
Another choice for comparing lineups is to calculate an empirical $p$-value from the empirical distribution of the distance metric. This would enable comparison of all the metrics for any lineups. It is time-consuming to calculate for a large number of lineups.
