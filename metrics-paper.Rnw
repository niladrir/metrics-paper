...
prop.dist <- dd %>% group_by(pic_name) %>%
  summarize(diff.box = box.mean[len == 19] - max(box.mean[len == 18]), grtr.box = sum(box.mean[len == 18] > box.mean[len == 19]) , diff.bin = bin.mean[len == 19] - max(bin.mean[len == 18]), grtr.bin = sum(bin.mean[len == 18] > bin.mean[len == 19]) , prop = mean(prop), m.time = mean(m.time))
...
qplot(value, prop, data = prop.diff.m, geom = "point", size = I(3), 
      ylim = c(0, 1), xlab = "Difference", ylab = "Detection Rate", 
      shape = factor(special)) + 
