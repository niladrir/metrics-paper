...
\section{Distance Measures for Lineups}
\subsection{Distance Measures} \label{sec:meas}

\hh{XXX Plan for this section: introduce distance measures. These are chosen such that for each of the studies we discuss later on we have one general distance for all of them and one distance metric that is tailored specifically to the situation of the study (which is still fairly general because it mostly depends on the plot type). }
...
\hh{XXX Minimum separation seems to perform a bit closer to what observers see. We could work this point out a bit more. }
...
\section{Description of Experimental Results and Analysis of Metrics} \label{sec:results}
...
In Figure~\ref{turk1comp},  detection rate is plotted against $\delta$-difference. The  vertical line highlights a  difference of zero indicating that the mean distance of the true plot is equal to the maximum of the mean distance of the null plots i.e.\ the mean distance of the true plot is equal to at least one of the mean distance of the null plots. As the $\delta$-difference increases, detection rate generally increases. %So the subjects do better in the easier lineups than the hard ones. 
\hh{The solid lines in Figure~\ref{fig:turkcomp-1} are fits  from  logistic regression models using a quadratic effect for distance.
...
\hh{If participants base their choice on graphical elements, binned distance might therefore not be able to adequately reflect this.}
\hh{In order to investigate on what participants base their choice, we are going to have a closer look at an individual lineup. }
...
\hh{XXX We need a few more specific details: }
...
The motivation behind this experiment is to study the effect of \hh{high dimensions} on separability in data. Scenarios with pure noise and some real separation in two or three groups were investigated. %Data was simulated with different dimensions and fixed sample size. Data was divided into two or three groups. 
