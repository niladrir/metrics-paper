...
and binned distance (AIC: \Sexpr{round(AIC(dbn), 1)}). \hh{While motivated from cognitive principles, Dunn separation comes in at a  surprising(?) last place (AIC: \Sexpr{round(AIC(ddunn), 1)}).  }
