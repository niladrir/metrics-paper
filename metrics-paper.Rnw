...
lp2 <- read.table("data/dat_turk2_100_75_5_3.txt", header=TRUE)
mlp2 <- melt(lp2, id.var="X", measure.var=2:21)
mlp2$variable <- as.numeric(gsub("X", "", mlp2$variable))

qplot(X, value, data=mlp2, alpha=I(0.5), facets=~variable, size=I(2.5)) + 
  geom_smooth(method="lm", se=FALSE, size=1.25, alpha=0.05) +
  theme_lineup + theme(aspect.ratio=1)
@
\end{subfigure}
...
<<lp-turk2-distr, echo=FALSE, cache=TRUE>>=
# use replicate
@

<<hist-reg, dependson='lp-reg', echo=FALSE>>=
...
<<lp-box, echo=FALSE, fig.width=6, fig.height=6, out.width='\\textwidth'>>=
...
df <- data.frame(t(replicate(1000, {
  	samp.dat <- data.frame(group = obs.dat$group, 
  	                       val = rnorm(dim(obs.dat)[1], 0, 11.77))
...
#   P <- subset(lineup.dat, .sample == metrics.dat$pos.1[i])
#   PX <- subset(lineup.dat, .sample == metrics.dat$pos.2[i])
#     
...
<<lp-box-dist, dependson='lp-box-dist-simu', echo=FALSE,  fig.width=4.5, fig.height = 4, out.width='\\textwidth'>>=

ymax <- max(density(df$box)$y)
ggplot()  + 
  geom_density(data = df, aes(x = box), 
               fill = "grey80", col = "grey80") +
  xlab("Boxplot Based Distance") + ylab("") + 
...
#                size=1, alpha = I(0.7)) + 
#    + 
#   geom_text(data = dd3, y = - 0.03*ymax, size = 2.5, 
