...
\hh{XXX I'm not terribly happy with the next paragraph - it  might be because I don't quite see the storyline that it is supposed to fit into. One suggestion for the story would be `ideally, while distances based on graphical elements do better, binned distance would be a general approach independent of plot type and specific question, that does relatively well. Unfortunately, its limitations are quite restrictive. For turk1 binned distance performs almost as well as boxplot based distance. turk2 shows that binned distance is not able to deal well with large variability. The number of bins in X and Y direction is affecting the outcome massively.' }
...
Figure~\ref{turk1-exp} shows the lineup and the distribution of the distance metrics for a closer look at what might observers lead to pick the data plot as different. \hh{The grey numbers at the bottom right of each of the plots in the lineup gives an overview of which plots participants picked in \Sexpr{nrow(lpsub)} evaluations of the lineup.} 
...
dframe <- data.frame(t(replicate(100, {
	samp.dat <- data.frame(group = obs.dat$group, 
	                       val = rnorm(nrow(obs.dat), 0, 5))
  dat1 <- data.frame(t(replicate(18, {
    null.dat <- data.frame(group = obs.dat$group, 
                           val = rnorm(nrow(obs.dat), 0, 5))
    b1 = bin_dist(X=samp.dat, PX=null.dat, lineup.dat = lineup.dat, 
...
ggplot()  + geom_density(data = metrics.dat, aes(x = bx), 
                         fill = "grey80", col = "grey80" ) + 
  geom_segment(data = subset(dd1, len == 19), 
               aes(x= bx, xend = bx, 
                   y=0.02*max(density(metrics.dat$bx)$y), 
                   yend = 0.2*max(density(metrics.dat$bx)$y)), 
               colour="darkorange", size=1)  + 
  geom_segment(data = subset(dd1, len != 19), 
               aes(x = bx, xend = bx, 
                   y = rep(0.02*max(density(metrics.dat$bx)$y),19), 
                   yend = rep(0.1*max(density(metrics.dat$bx)$y),19)), 
               size=1, alpha = I(0.7)) + xlab("Boxplot Based Distance") + 
  ylab("") + geom_text(data = dd1, y = - 0.03*max(density(metrics.dat$bx)$y), 
                       size = 2.5, aes(x = bx, label = pos.1)) + 
  ylim(c(- 0.04*max(density(metrics.dat$bx)$y), 
         max(density(metrics.dat$bx)$y) + 0.1*max(density(metrics.dat$bx)$y)))
@
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
...
ggplot()  + geom_density(data = metrics.dat, aes(x = b28), 
                         fill = "grey80", col = "grey80" ) + 
  geom_segment(data = subset(dd1, len == 19), 
               aes(x= b28, xend = b28, 
                   y=0.02*max(density(metrics.dat$b28)$y), 
                   yend = 0.2*max(density(metrics.dat$b28)$y)), 
               colour="darkorange", size=1)  + 
  geom_segment(data = subset(dd1, len != 19), 
               aes(x = b28, xend = b28, 
                   y = rep(0.02*max(density(metrics.dat$b28)$y),19), 
                   yend = rep(0.1*max(density(metrics.dat$b28)$y),19)), 
               size=1, alpha = I(0.7)) + xlab("Binned Distance (p = 2, q = 8)") + 
  ylab("") + geom_text(data = dd1, y = - 0.03*max(density(metrics.dat$b28)$y), 
                       size = 2.5, aes(x = b28, label = pos.1)) + 
  ylim(c(- 0.04*max(density(metrics.dat$b28)$y), 
         max(density(metrics.dat$b28)$y) + 0.1*max(density(metrics.dat$b28)$y)))
@
\end{subfigure}
\begin{subfigure}[t]{0.32\textwidth}
...
ggplot()  + geom_density(data = metrics.dat, aes(x = b22), 
                         fill = "grey80", col = "grey80" ) + 
  geom_segment(data = subset(dd1, len == 19), 
               aes(x= b22, xend = b22, 
                   y=0.02*max(density(metrics.dat$b22)$y), 
                   yend = 0.2*max(density(metrics.dat$b22)$y)), 
               colour="darkorange", size=1)  + 
  geom_segment(data = subset(dd1, len != 19), 
               aes(x = b22, xend = b22, 
                   y = rep(0.02*max(density(metrics.dat$b22)$y),19), 
                   yend = rep(0.1*max(density(metrics.dat$b22)$y),19)), 
               size=1, alpha = I(0.7)) + xlab("Binned Distance (p = q = 2)") + 
  ylab("") + geom_text(data = dd1, y = - 0.03*max(density(metrics.dat$b22)$y), 
                       size = 2.5, aes(x = b22, label = pos.1)) + 
  ylim(c(- 0.04*max(density(metrics.dat$b22)$y), 
         max(density(metrics.dat$b22)$y) + 0.1*max(density(metrics.dat$b22)$y)))
@
\end{subfigure}
...
\hh{XXX the distributions for the distances seems to be off by the standard deviation setting in the simulation. It should be sd=12, which is what the lower row of density plots is using. Given the lineup they look more realistic to me than the upper row - given the small difference in the medians, the distance measures should not be very extreme. This would make a good example for using the empirical $p$-values as a more objective measure of lineup difficulty. }
\hh{XXX paragraph below needs some cleaning up. }
...
In this experiment, the test statistic is a scatterplot with the regression line overlaid. Assuming that the null hypothesis is true, the null plots are generated by assuming that there is no significant linear relationship between the two variables. The subjects were shown a set of ten lineups each and were asked to identify the plot with the steepest slope. 
% Figure~\ref{lineup-example,dist,turk2} show example lineups. 
...
The distances between the plots in this experiment were computed using both the distance based on regression line ($d_{RG}$) and the binned distance ($d_{BN}$) with a small number of bins. The proportion of correct response for each lineup was calculated from the response of the subjects and plotted against $\delta$ and $\gamma$.  Figure~\ref{turk2comp} shows the results for the distance based on the regression line and the binned distance against $\delta$.
...
%\label{t1bin_1}
%}
<<turk2-time, dependson='delta-turk2', echo=FALSE, fig.width=8.5, fig.height=4.5, out.width='0.7\\textwidth'>>=
ggplot(data = prop.diff.m) + 
