...
\hh{We investigated another generic distance, the }
...
\hh{For each of the levels in $X_1$ find the corresponding $X_2$ values, and calculate first quartile, median and the third quartile. Identify the minimum and maximum value of each of these statistics across all $J$ groups.}
...
\end{eqnarray}
\hh{XXX intercept and slope could be on very different scales. In a scatterplot we usually aim for an aspect ratio of 1, so a deviation in X is perceived to be about the same as a deviation in Y (even though we could read the axis, people usually don't). This could be helped with by scaling X and Y first. XXX }
...
How well each distance measure matches the observers' responses depends  on the question of interest. But, in general, this should not be a problem because the question which is typically asked is ``Which plot among these is different?''. In the MTurk experiments, more focused questions were asked because the purpose was very specific, to compare visual inference with classical inference.
